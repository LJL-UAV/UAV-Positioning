...
for i = 1:length(folders)
    ...
    if exist(str1, 'file') && exist(str2, 'file')
        ...
        %% 显示匹配结果
        figure;
        showMatchedFeatures(image1, image2, cleanedPoints1(:,1:2), cleanedPoints2(:,1:2), 'montage');
        % 获取当前活动的 axes 对象，确保 line 添加到正确的地方
        ax = gca;
        hold on;  % 保持图像
        % 获取图像的宽度，用于调整第二幅图像中点的位置
        imageWidth = size(image1, 2);
        
        % 手动绘制 finalPoints 匹配
        for k = 1:size(finalPoints1, 1)
            % 绘制连接线
            line([finalPoints1(k,1), finalPoints2(k,1) + imageWidth], [finalPoints1(k,2), finalPoints2(k,2)], ...
                'Color', 'blue', 'LineWidth', 0.5, 'Parent', ax);
            % 绘制特征点
            plot(ax, finalPoints1(k,1), finalPoints1(k,2), 'go', 'MarkerSize', 1, 'LineWidth', 1);
            plot(ax, finalPoints2(k,1) + imageWidth, finalPoints2(k,2), 'mo', 'MarkerSize', 1, 'LineWidth', 1);
        end
        hold off;

        %% 保存匹配图像
        folderPath = 'F:\PC2-Data\UAV_GE1\Pairs_new4\results\YX\LNIFT';  % 你可以将其修改为任何具体的文件夹路径，例如 'C:\Users\YourUsername\Documents\Matlab\a'
        % 检查文件夹是否存在，如果不存在则创建
